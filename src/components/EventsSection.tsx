...
export default function EventsSection() {
    ...
    return (
        <section className="w-full bg-[#E6DDD8] pt-28 pb-16 relative px-4 md:px-8 flex flex-col justify-center overflow-hidden">
            {/* Reduced max-width to make grids smaller */}
            <div className="max-w-4xl mx-auto w-full flex flex-col">
                <h2 className={`${inter.className} font-bold text-[#4F3457] text-2xl md:text-4xl mb-6 mt-[30px] lowercase tracking-tight shrink-0`}>
                    our events
                </h2>

                {/* Glass Container */}
                <div
                    className="relative rounded-[2rem] md:rounded-[0.5rem] overflow-hidden p-4 md:p-6 flex flex-col"
                    style={{
                ...
                >
                    ...
                    <div className="relative z-10 w-full flex flex-col">
                        ...
                        <div className="flex justify-center items-center gap-4 mb-4 shrink-0">
                            <button
                                onClick={prevMonth}
                                className={`${inter.className} text-[#4F3457] text-xl md:text-3xl font-bold hover:opacity-70 transition-opacity`}
                            >
                                &lt;
                            </button>
                            <h3 className={`${inter.className} text-[#4F3457] text-xl md:text-3xl font-bold tracking-tight`}>
                                {monthName}
                            </h3>
                            <button
                                onClick={nextMonth}
                        ...
                        <div className="w-full flex flex-col">
                            ...
                            <div className="grid grid-cols-7 gap-1 mb-1 shrink-0">
                                {weekDays.map((day) => (
                                    <div key={day} className={`${inter.className} text-[#4F3457] text-[10px] md:text-sm font-bold uppercase text-center`}>
                                        {day}
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="grid grid-cols-7 gap-0 border-t border-l border-[#4F3457]/20 rounded-lg overflow-hidden">
                                {days.map((date, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className={`${inter.className} text-[#4F3457] font-bold text-xs md:text-base`}>
                                                {dayNumber}
                                            </span>
                                            ...
                                            {event && (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <h4 className={`${inter.className} text-[#4F3457] font-bold text-[10px] md:text-xs leading-tight line-clamp-2`}>
                                                            {event.event_title}
                                                        </h4>
                                                        ...
                                                        {event.link && (
                                                            <span className={`${inter.className} text-[#8F7CB7] text-[8px] font-bold underline`}>
                                                                RSVP
                                                            </span>
                                                        )}
                                                    </div>
